<#
  ...
  Stages references.json.gz, builds api (bakes packed.bin) + lb-nft for amd64.
...
$lbV  = "docker.io/$User/rinha-lb-nft:$Tag";  $lbL  = "docker.io/$User/rinha-lb-nft:latest"

$out = if ($Push) { "--push" } else { "--load" }
docker buildx build --platform linux/amd64 -f Dockerfile.api   -t $apiV -t $apiL $out $root
docker buildx build --platform linux/amd64 -f Dockerfile.lbnft -t $lbV  -t $lbL  $out $root
